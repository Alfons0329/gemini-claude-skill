...
Two follow-ups came out of an adversarial review of the change itself, and both were gaps rather than polish:

5. **`Action:` in `<id>-qa.md` is a command, not a sentence.** The file said "a cold reader can execute," which a prose step technically satisfies and practically does not. It matters more now than it did before: the coverage gap is a **diff between two plans**, and prose matches everything and nothing. `<id>-qa-adv.md` already required executable form at Layer 2, so the asymmetry was a hole on `impl`'s side only. A prose case reaching `verify` is triaged as a wrong test step, never counted as covered.
6. **A `fix-qa` diff is bounded by the failure it repairs.** Public interfaces stay put, a passing path keeps its logic, unimplicated modules are not touched. Frozen tests stop a session lowering the bar; nothing stopped it clearing the bar by rebuilding the wall. A session told to make one boundary case pass will otherwise reach for the redesign that makes it elegant, and take working behaviour with it. Where a fix genuinely needs a public interface changed, that is a design decision — reported and waited on, not applied.
