...
Builds shared mutual understanding between an engineer and the AI Assistant (Claude Code, Gemini CLI, Antigravity, etc.) before work starts. This skill ensures that non-obvious decisions, constraints, and assumptions are surfaced and documented.

## Core Principles

- **Read Before Asking:** Silently analyze all available context (`MEMORY.md`, `CLAUDE.md`, `GEMINI.md`, provided docs, related code via file search) to avoid redundant questions.
...
- Read project context files (e.g., `MEMORY.md`, `CLAUDE.md`, `GEMINI.md`).
- Read the initial input spec or ticket.
- Use web fetching tools (`web_fetch`, `read_url_content`, or `WebFetch`) for any provided URLs.
- Search the codebase for similar patterns using search tools (`grep_search`, `Grep`, or file search).
...
Ask questions in rounds using interactive question prompt tools (`ask_user`, `ask_question`, or direct user prompts).
